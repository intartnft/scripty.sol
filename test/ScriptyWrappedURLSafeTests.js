...
const expectedResultsPath = __dirname + "/expectedResults/ScriptyWrappedURLSafe/";
...
describe("ScriptyWrappedURLSafe Tests", function () {
    ...
    async function assertHTML(title, contract, headRequests, scriptRequests) {
        ...
        const htmlRaw = await contract.getHTMLWrappedURLSafe(htmlRequest)
        const htmlRawString = utilities.bytesToString(htmlRaw)

        expectHTML(title, htmlRawString);
    }

    describe("Wrapped URL Safe HTML Tests - Only contract scripts", async function () {
        it("Only contract scripts - wrapType = 0", async function () {
            ...
            await assertHTML(this.test.fullTitle(), scriptyWrappedURLSafeContract, headRequests, scriptRequests);
        });
    ...
    describe("Wrapped Wrapped URL Safe HTML Tests - Scripts with content", async function () {
    ...
    describe("Wrapped URL Safe HTML Tests - Contract scripts + scripts with content", async function () {
